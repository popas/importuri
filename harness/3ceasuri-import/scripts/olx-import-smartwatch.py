...
#
# Env:
#   AD_ID         numeric OLX ad id (the `id` from olx-find-smartwatches.py)
...
# is which watch this is. The model name and the generation come off the photos
# and the title, and that is what pass 1 hands you.
#
...
OLX_TAB = olx_api.ensure_tab(bu, olx_api.CATEGORY_URL[olx_api.CATEGORY_SMARTWATCH], tabs)
...
data = dict(mapped)
# A smartwatch listing is a smartwatch: these three are not inferred, they are the
# category. Leaving `movement` to the harness default would silently write quartz.
data.update({"movement": "smart", "style": "smart", "displayType": "smart",
             "category": "wrist"})
if brand:
    data["brand"] = brand
...
# The three category facts survive the clearing — a smartwatch stays a smartwatch
# even when the answer forgets to restate them. An answer that actively disagrees
# is a routing mistake, not a correction, so it stops instead.
if data.get("movement") not in (None, "smart"):
    emit("REVIEW", {"ad_id": AD_ID, "reasons": [
        "movement=%r on the smartwatch importer — if this is a mechanical/quartz watch, "
        "import it with olx-import-watch.py instead" % data.get("movement")]})
    if not CONFIRM:
        raise SystemExit(0)
for f, v in (("movement", "smart"), ("style", "smart"), ("displayType", "smart")):
    data.setdefault(f, v)
data.setdefault("category", "wrist")

if OVERRIDES.get("is_wristwatch") is False and not CONFIRM:
    emit("SKIP", {"ad_id": AD_ID,
                  "reason": "not a watch (accessory: strap/charger/case/box); "
